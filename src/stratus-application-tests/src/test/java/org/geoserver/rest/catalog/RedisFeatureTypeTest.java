...
@SpringBootTest(classes = {
        ...
        /* Cache preloaders */
        WebXmlConfig.class, RestCachingInterceptor.class, FeatureTypePreloader.class, FeatureTypesPreloader.class},
        properties = {"stratus.catalog.redis.caching.enable-rest-caching=true", "spring.main.allow-bean-definition-overriding=true"})
public class RedisFeatureTypeTest extends FeatureTypeControllerTest {
    ...
    /*
    Apparently when the initial featuretype for this test is created, it's created without attributes, but when the json
    is posted to the FeatureTypeController, that data is deserialized with attributes.  The objects are thus not equal and
    the test fails.  It seems that any attempt to actually make the two objects equal results in a stack overflow during
    the comparison.
     */
    @Test
    @Override
