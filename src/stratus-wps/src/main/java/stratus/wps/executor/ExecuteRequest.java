...
public class ExecuteRequest {
    ...
    public ExecuteRequest(org.geoserver.wps.executor.ExecuteRequest request){
        this.request = request.getRequest();
        processName = Ows11Util.name(request.getRequest().getIdentifier());
        pf = GeoServerProcessors.createProcessFactory(processName, true);
        if(pf == null) {
            throw new WPSException("Unknown process " + processName);
        }
    ...
    public LazyInputMap getProcessInputs(StratusWPSExecutionManager manager) {
        if (inputs == null) {
    ...
    LazyInputMap getInputsInternal(StratusWPSExecutionManager manager) {
